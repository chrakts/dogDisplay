...
//User functions to write raw data from flash
  ...
  void lcd_draw_image_P(const char *progmem_image PROGMEM, uint8_t pages, uint8_t columns, uint8_t style);
  void lcd_draw_image_xy_P(const char *progmem_image PROGMEM, uint8_t x, uint8_t y, uint8_t pages, uint8_t columns, uint8_t style);
#endif
...
void lcd_bar_graph_xy(uint8_t pages, uint8_t columns, uint8_t style, uint8_t page, uint8_t col, uint8_t percentage);
...
#define LCD_DRAM()            LCD_A0_PORT.OUTSET = LCD_A0_PIN // PORT_A0 |= _BV(PIN_A0)
#define LCD_CMD()             LCD_A0_PORT.OUTCLR = LCD_A0_PIN // PORT_A0 &= ~_BV(PIN_A0)
//#define LCD_SET_OUTPUT_A0()   DDR_A0  |= _BV(PIN_A0)

//Control reset input of LCD
#define LCD_RESET_OFF()       LCD_RST_PORT.OUTSET = LCD_RST_PIN // PORT_RST |= _BV(PIN_RST)
#define LCD_RESET_ON()        LCD_RST_PORT.OUTCLR = LCD_RST_PIN // PORT_RST &= ~_BV(PIN_RST)
...
//Control pin for Backlight
  ...
  #define BACKLIGHT_ON()       LCD_BACKL_PORT.OUTSET = LCD_BACKL_PIN // PORT_LED |= (1<<PIN_LED)
  #define BACKLIGHT_OFF()      LCD_BACKL_PORT.OUTCLR = LCD_BACKL_PIN // PORT_LED &= ~(1<<PIN_LED)
#elif LCD_USE_BACKLIGHT == 2    //inverted
  //#define LCD_SET_OUTPUT_LED() DDR_LED |= _BV(PIN_LED)
  #define BACKLIGHT_ON()       LCD_BACKL_PORT.OUTCLR = LCD_BACKL_PIN // PORT_LED &= ~(1<<PIN_LED)
  #define BACKLIGHT_OFF()      LCD_BACKL_PORT.OUTSET = LCD_BACKL_PIN // PORT_LED |= (1<<PIN_LED)
#else
...
//Control pin for chip select
  ...
  #define LCD_SELECT()         LCD_CS_PORT.OUTCLR = LCD_CS_PIN
  //#define LCD_UNSELECT()       spi_wait_for_idle(); LCD_CS_PORT.OUTSET = LCD_CS_PIN
  #define LCD_UNSELECT()       LCD_CS_PORT.OUTSET = LCD_CS_PIN
#else
...
#define LCD_SET_PIN_DIRECTIONS()   LCD_A0_PORT.DIRSET = LCD_A0_PIN; \
                                   LCD_RST_PORT.DIRSET = LCD_RST_PIN; \
                                   LCD_BACKL_PORT.DIRSET = LCD_BACKL_PIN; \
                                   LCD_CS_PORT.DIRSET = LCD_CS_PIN
