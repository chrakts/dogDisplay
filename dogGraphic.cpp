 ...
 * col           - column of upper left corner
 * page          - page of upper left corner
 * percentage	 - percentage of dark area
 * Cursor is moved to start of area after clear
 */
...
uint8_t grenze;
  lcd_moveto_xy(page,col);
  grenze = (uint8_t)((columns*percentage)/100);
  lcd_clear_area(pages,grenze,INVERT_BIT);
  lcd_moveto_xy(page,col+grenze);
  lcd_clear_area(pages,columns-grenze,0);
}
